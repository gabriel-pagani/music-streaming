...
CREATE TABLE SOLICITACOES (
	...
	STATUS VARCHAR(10) CHECK (STATUS IN ('Em análise', 'Aprovada', 'Reprovada')) DEFAULT 'Em análise',
	DATA_SOLICITACAO DATE DEFAULT GETDATE(),    
	OBSERVACOES VARCHAR(MAX), 	

	CONSTRAINT CHK_DATAS CHECK (
		DATEADD(DAY, 30, DATA_EMPRESTIMO) > DATA_EMPRESTIMO
		AND DATA_EMPRESTIMO > CAST(DATEADD(DAY, 15, GETDATE()) AS DATE)
	)
);
...
CREATE TABLE EMPRESTIMOS (
	...
	ID_SOLICITACAO INT FOREIGN KEY REFERENCES USUARIOS(ID) NOT NULL,
	VALOR DECIMAL(10,2),
	JUROS_MENSAL DECIMAL(5,2),
	DATA_EMPRESTIMO DATE NOT NULL,
	DATA_VENCIMENTO DATE NOT NULL,
    	EXPLICACAO VARCHAR(MAX) NOT NULL, 
	STATUS VARCHAR(8) CHECK (STATUS IN ('Em andamento', 'Quitado', 'Atrasado')) DEFAULT 'Em andamento', 
	OBSERVACOES VARCHAR(MAX), 	
);
